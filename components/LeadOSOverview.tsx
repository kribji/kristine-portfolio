...
type LeadOSOverviewProps = {
  description: string;
};

export default function LeadOSOverview({ description }: LeadOSOverviewProps) {
    ...
    <div className="mt-4 space-y-8">
      {sections.map((section, index) => (
        <div key={`${section.label ?? "section"}-${index}`}>
          {section.label && (
            <p
              className="mb-2 font-sans text-[12px] uppercase tracking-[0.1em] text-muted"
            >
              {section.label}
            </p>
          )}
